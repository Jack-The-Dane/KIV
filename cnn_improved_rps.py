...
# CONFIG
# -----------------------------
DATA_DIR = "./GestureClassification"   # change to your dataset path
SAMPLING_RATE = 256  # gesture dataset sampling rate (Hz)
SEGMENT_LENGTH = 256+512  # samples (~4 seconds)
BATCH_SIZE = 32
EPOCHS = 50
LEARNING_RATE = 1e-3
...
def load_data(data_dir):
    # Load the training_data.csv you generated (or the raw PaperData.csv)
    df0 = pd.read_csv(data_dir + "/" + DATA1, header=None)
    df1 = pd.read_csv(data_dir + "/" + DATA2, header=None)
    ...
    # Separate features (the 1024 points) and label
    # Assuming 'label' is the last column
    X_raw = df.drop(columns=['label']).to_numpy()
    y = df['label'].to_numpy()
    
    # Reshape to (Samples, 1, 1024)
    X_reshaped = X_raw.reshape(-1, 1, 1024)
...
# SEGMENTATION
# -----------------------------
def segment_signal(signal, segment_length, overlap=0.8):
...
def create_dataset(signals, labels):
    ...
    for sig, label in zip(signals, labels):
        segments = segment_signal(sig[0], SEGMENT_LENGTH)
